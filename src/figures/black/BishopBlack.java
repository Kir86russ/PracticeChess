...
public class BishopBlack implements Figure {
    ...
    // Проверка на атаку своего короля
    ...
    public Cell getPosition() {
        ...
        // Проверка на шах чужому королю после нашего хода (именно для текущей фигуры)
    ...
    public Cell[] way(Cell source, Cell dest, Figure[] figures) {
        ...
        if (source.x == dest.x + 1 && source.y == dest.y - 1) { /////////////////////// вниз влево
            if (Logic.isShah) {
                for (int i = 0; i != figures.length; i++) {
                    if (figures[i] != null)
                    if (figures[i].getPosition() == dest && figures[i].getColor() == "white") {
